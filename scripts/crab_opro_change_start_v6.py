# v6: generalised dedup with PARAM_BOUNDS/PARAM_SCALES config
...
# ── Stall detection ───────────────────────────────────────────────────────────
def is_stalled(history: list, n: int = 3) -> bool:
    if len(history) < n:
        return False
    recent = history[-n:]
    return all(
        max(t[p] for t in recent) - min(t[p] for t in recent) <= STALL_TOL
...
def llm_propose(history: list) -> dict:
    ...
    grad = estimate_gradient(history, best, GRAD_EPS)
...
# ── Hyperparameter config (tune all of these when switching source/dataset) ────
PARAM_BOUNDS = {                        # hard physical bounds per parameter
    "log_k": (-44.0, -19.0),
    "alpha":  ( -4.0,   2.0),
    "beta":   ( -4.0,   2.0),
}
PARAM_SCALE_DIVISOR = 25.0             # step size = range / divisor; increase for sharper surfaces
PARAM_SCALES = {k: (hi - lo) / PARAM_SCALE_DIVISOR for k, (lo, hi) in PARAM_BOUNDS.items()}

GRAD_EPS = {                            # neighbor search window for finite-difference gradient
    "log_k": 2.0,                       # ~8% of log_k range
    "alpha": 0.5,                       # ~8% of alpha range
    "beta":  0.25,                      # ~4% of beta range
}

LOGLIKE_FLOOR = -5e5                   # evaluations below this are treated as failed; good region ~[-18000, -25000]
STALL_TOL     = 0.05                   # max per-parameter spread across last 3 proposals before declaring a stall


def clip(proposal: dict) -> dict:
    return {k: max(lo, min(hi, proposal[k])) for k, (lo, hi) in PARAM_BOUNDS.items()}


def dedup(proposal: dict, scales: dict = PARAM_SCALES, max_tries: int = 8) -> dict:
    for attempt in range(max_tries):
        key = tuple(round(proposal[k], 6) for k in scales)
        if key not in logL_cache:
            return proposal
        base = 1.0 * (2 ** attempt)
        proposal = clip({k: proposal[k] + random.gauss(0, base * scales[k]) for k in scales})
        print(f"  [dedup retry {attempt+1}, scale={base:.2f}]")
    return proposal
...
for i in range(N_ITERATIONS):
    if is_stalled(history):
        ...
        proposal = dedup(clip({
            k: best_pt[k] + random.gauss(0, PARAM_SCALES[k])
            for k in PARAM_SCALES
        }))
