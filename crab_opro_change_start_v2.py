# v2: robust JSON parsing, stall detection, top-15-by-logL context
...
def parse_llm_response(text: str) -> dict | None:
    ...
    if match:
        try:
            ...
            pass
    # Fallback: pull values by key name when JSON is malformed
...
def llm_propose(history: list) -> dict:
    ...
    # Top-15 by logL (fix 3) — LLM always sees the best-explored region
    top = sorted(
        [t for t in history if t["logL"] > LOGLIKE_FLOOR],
        key=lambda x: x["logL"], reverse=True
    )[:15]
    history_str = "\n".join(
...
Recommended step sizes per iteration (scale down if recent steps are not improving):
  log10(K): ±0.3   alpha: ±0.05   beta: ±0.02
Move in the gradient direction. Do NOT jump more than 2x the recommended step size.

Top-15 evaluated points so far (best first):
{history_str}
